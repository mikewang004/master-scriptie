...
sub CLOSE_DATA_FILE { close(DATA); }; 
    ...
    if ($accept_branched eq 1) {
        foreach $i (1 .. $maxid) { 
            if ($conns[$i] eq 3) { 
                ...
                @tmp1=split(/ /,$conn[$i]); $conn[$i]=""; foreach $itmp1 (0 .. $#tmp1-1) { $conn[$i].="$tmp1[$itmp1] "; }; 
                $conns[$i]-=1; 
                $ii = $tmp1[$#tmp1]; 
                ...
                die 'four-functional junctions: not implemented. contact mk';
            }; 
        }; 
...
sub CLOSE_DUMP_FILE { close(DUMP); }; 
